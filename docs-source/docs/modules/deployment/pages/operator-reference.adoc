...
[source,yaml,title='shopping-cart-service.yml']
----
...
[source,yaml,title='kustomization.yml']
----
resources:
- shopping-cart-service.yml
----

Deploy your default service by running:
...
Create a separate folder along `base` and call it `replicas` with this content:

The actual customization:
[source,yaml,title='replicas.yml']
----
apiVersion: akka.lightbend.com/v1
kind: AkkaMicroservice
metadata:
...
Scale the number of pods by applying the overlaid deployment descriptor by running:

[source,shell script]
----
kubectl apply -k replicas
----

Kubernetes will reconcile the number of Pods to the desired scale.
...
Create a separate folder along `base` and call it `resources` with this content:

The actual customization:
[source,yaml,title='resources.yml']
----
apiVersion: akka.lightbend.com/v1
kind: AkkaMicroservice
metadata:
...
Apply the new resources and limits by running:
...
A custom logback configuration can be provided to the deployment, make sure that your application is already configured to https://doc.akka.io/docs/akka/current/logging.html#slf4j[use Slf4j for logging] .
Create a separate folder along `base` and call it `logback` with this content:

The actual customization:
[source,yaml,title='logback.yml']
----
apiVersion: akka.lightbend.com/v1
kind: AkkaMicroservice
metadata:
...
the reference to the customization:
[source,yaml,title='kustomization.yml']
----
bases:
- ../base
...
and the logback configuration to use:
